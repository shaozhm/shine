sap.ui.controller("sap.hana.democontent.epm.salesdashboard.view.details", {
	...
	onRowSelect: function(oEvent) {
		...
		// var selected = oSHTable.getSelectedIndex();
		// var index = oSHTable.getContextByIndex(selected);
		// var data = index.getObject();
		// var targetCurrency = data.CURRENCY;

		// var targetCurrency = sap.ui.getCore().byId("__xmlview0--currencyDropdown").getValue();

		var sPath = this.byId("soHeader").getModel().sServiceUrl;
		console.log(sPath);
		var expr = "currencyConversion";
		var flag = sPath.match(expr);
		...
		if (!flag) {
			...
			var targetCurrency = sap.ui.getCore().byId("__xmlview0--currencyDropdown").getValue();
			console.log("Converted currency : " + targetCurrency);
			var oSHTable = this.byId("soHeader");
			var selected = oSHTable.getSelectedIndex();
			var index = oSHTable.getContextByIndex(selected);
			var data = index.getObject();
			...
			var oModel = new sap.ui.model.odata.ODataModel("/sap/hana/democontent/epm/services/currencyConversion.xsodata/", true);
			var oSITable = sap.ui.getCore().byId("details--soItemTable");
			...
			oSITable.bindRows({
				path: "/CurrencyInput1(IP_1='" + targetCurrency + "')/Execute?$filter=SALESORDERID eq '" + SALESORDERID + "'",
				// parameters: {
				// 	//  expand: "Buyer",
				//   	select: "SALESORDERID,CURRENCY,GROSSAMOUNT,TAXAMOUNT,PARTNERID,COMPANYNAME,CITY,PARTNER_PARTNERID"
				// },
				sorter: sort1
			});
		}
	...
	onRefresh: function(oEvent) {
		...
		$.ajax({
			...
			success: function(data, textStatus, request) {
				oSHTable.setTitle("Sales Orders" + " (" + data.toString() + ")");
		...
		var sort1 = new sap.ui.model.Sorter("SALESORDERID", true);
		var sPath = oSHTable.getModel().sServiceUrl;
		console.log(sPath);
		var expr = "currencyConversion";
		var flag = sPath.match(expr);
		...
		if (flag) {
			var targetCurrency = sap.ui.getCore().byId("__xmlview0--currencyDropdown").getValue();
			path = "/CurrencyInput(IP_1='" + targetCurrency + "')/Execute";
	...
	onSubmit: function(oEvent, min, max) {
		...
		//if(this.currencyIsEnabled()){
			this.currencyIsEnabled();
	//	}
	},
currencyIsEnabled: function(){
	var that = this;
   	$.ajax({
		type: "GET",
		url: "/sap/hana/democontent/epm/services/currencyConversionConfig.xsjs?cmd=isEnabled",
		contentType: "application/json",
		success: function(data) {
			if(data.ISENABLED){
			//return true;
			that.fetchLatestExchangeRate();
			}
		},
		error: function(error){
				sap.ui.commons.MessageBox.show('Error occurred in checking currency conversion config.',
					"ERROR",
					"Currency Conversion");
...
fetchLatestExchangeRate: function(){
	var	BusyDialog =  new sap.m.BusyDialog({
					title:"Currency Conversion",
					text:"Fetching latest currency rates ...",
					showCancelButton:false
					}).open();
		$.ajax({
			type: "GET",
			url: "/sap/hana/democontent/epm/services/updateTcurrTable.xsjs?cmd=SOCreate",
			contentType: "application/json",
			success: function(data) {
				if(data==="SUCCESS" || data.message==="nodata" || data.message==="uptodate"){
					BusyDialog.close();
				//	alert(data.message);
				}
				if(data.message==="error" ){
					BusyDialog.close();
					sap.ui.commons.MessageBox.show('Error : '+JSON.stringify(data.error),
					"ERROR",
					"Currency Conversion");
			...
			error: function(error){
				BusyDialog.close();
				console.log("Error in saving client_id and client_secret");
                  	sap.ui.commons.MessageBox.show('Error in fetching currency rates.',
					"ERROR",
					"Currency Conversion");
            }
		});
	...
	onInit: function() {
		...
		//  window.oComboBoxPd = new sap.ui.commons.ComboBox({
...
sap.ui.core.BusyIndicator.show(0);
		...
		if (oSHTable != undefined) {
			...
			oSHTable.bindRows({
				...
				parameters: {
					//  expand: "Buyer",
					select: "SALESORDERID,CURRENCY,GROSSAMOUNT,TAXAMOUNT,PARTNER_PARTNERID,COMPANYNAME,CITY,HISTORY_CREATEDAT"
				},
				sorter: sort1
			});
		...
		// 		localStorage.setItem("detailsEnabled", "true");
		// 		if(localStorage.currencyEnabled==="true" && localStorage.detailsEnabled==="true"){
		// 			sap.ui.getCore().byId("__xmlview0--btnPersonalize").setVisible(true);
		// 		}else{
		// 			sap.ui.getCore().byId("__xmlview0--btnPersonalize").setVisible(false);
		// 		}
		// }
		setInterval(this.validatePersonalize(),2000);
	},
	validatePersonalize: function() {
		$.ajax({
			type: "GET",
			url: "/sap/hana/democontent/epm/services/currencyConversionConfig.xsjs?cmd=isEnabled",
			contentType: "application/json",
			...
			error: function(error) {
				sap.m.MessageBox.show('Error occurred in currency enabling',
					"ERROR",
					"Currency Conversion");
	...
	createNewDialogContent: function(oController) {
		...
		view.oLayout.createRow(view.submitButton);
			//Product details xsodata url
		// var sPath = "/ProductDetails";
		// this.oComboBoxPd="";
		...
		//that.createNewLineItemContent(oController, oComboBoxPd);
		//******changing******
	...
	createNewLineItemContent: function(oController) {
		...
		this.oComboBoxPd.setModel(new sap.ui.model.odata.ODataModel("/sap/hana/democontent/epm/services/productDetails.xsodata",true));
